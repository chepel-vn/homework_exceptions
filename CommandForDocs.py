...
documents = [
  ...
  {"type": "passport", "number": "3004 897564", "name": "Геннадий Покемонов"},
...
directories = {
  '1': ['2207 876234', '11-2', '5455 028765', '11-3'],
  '2': ['10006', '5400 028765', '5455 002299'],
  '3': ['10007', '3004 897564', '10008'],
  '4': []
}
...
def execute_command_people():
  ...
  if document:
    ...
    if author_name:
      ...
      print(f"У документа \"{document_number}\" не прописан владелец(автор).")
  else:
  ...
  if len(documents) > 0:
    ...
    for document in documents:
      try:
        ...
        print('{0} \"{1}\" \"автор неизвестен\"'.format(document['type'], document['number']))
  else:
...
def execute_command_move():
  ...
  if shelf_number_old:
    ...
    if shelf_number_old != shelf_number_new:
      ...
      print("Документ \"{0}\" перемещен с полки \"{1}\" на полку \"{2}\".".format(document_number, shelf_number_old, shelf_number_new))
    else:
...
def execute_command_list_owners():
  """
    (None) -> None

    Function prints all owners of documents
  ...
  if len(documents) > 0:
    ...
    for document in documents:
      try:
        ...
        count_without_owner += 1
        # print(f"В документе \"{document['number']}\" отсутствует поле владельца(автора) документа.")
      authors.add(author)

    print("Cписок всех имен владельцев документов:")
    for element in authors:
      print(f"\"{element}\"")
    print(f"Но есть {count_without_owner} документов, у которых владелец не указан.")
  else:
    print("Каталог документов пуст.")
...
def execute_command(command_name = "p"):
  ...
  elif command_name == "lo":
    execute_command_list_owners()
...
def main():
  ...
  while True:
    command = input('Введите команду (l=list, p=people, s=shelf, a=add, d=delete, m=move, as=add shelf lo=list owners q=quit): ')

    if command == 'q':
      print("Вы выбрали команду \"q\", следовательно выходим из программы.")
      break
    elif command in ('p', 'l', 's', 'a', 'd', 'm', 'as', 'lo'):
